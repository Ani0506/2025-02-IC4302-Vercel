...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Normalize buckets to { value, count }
    ...
    // Fallback facet counts via basic aggregation if Atlas Search is unavailable
    const match: any = {}
    if (search) {
    ...
    const dateFacet = async () => {
      const pipeline = [
        { $match: match },
        // Move the field with space to a helper field to avoid path issues
